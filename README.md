# Storrik TypeScript API Library

[![NPM version](<https://img.shields.io/npm/v/storrik.svg?label=npm%20(stable)>)](https://npmjs.org/package/storrik) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/storrik)
...
```sh
npm install git+ssh://git@github.com:storrik/sdk.git
```
...
1. Via the `STORRIK_LOG` environment variable
...
const client = new Storrik({
  logger: logger.child({ name: 'Storrik' }),
...
const client = new Storrik({ fetch });
```
...
```ts
import Storrik from 'npm:storrik';
