...
import { APIPromise } from 'storrik/core/api-promise';

import util from 'node:util';
import Storrik from 'storrik';
import { APIUserAbortError } from 'storrik';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    test('debug logs when log level is debug', async () => {
      ...
      const client = new Storrik({ logger: logger, logLevel: 'debug', apiKey: 'My API Key' });
    ...
    test('default logLevel is warn', async () => {
      const client = new Storrik({ apiKey: 'My API Key' });
    ...
    test('debug logs are skipped when log level is info', async () => {
      ...
      const client = new Storrik({ logger: logger, logLevel: 'info', apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Storrik({ logger: logger, apiKey: 'My API Key' });
    ...
    test('client log level overrides env var', async () => {
      ...
      process.env['STORRIK_LOG'] = 'debug';
      const client = new Storrik({ logger: logger, logLevel: 'off', apiKey: 'My API Key' });
  ...
  test('normalized method', async () => {
    ...
    const client = new Storrik({ baseURL: 'http://localhost:5000/', apiKey: 'My API Key', fetch: testFetch });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Storrik({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['STORRIK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Storrik({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('empty env variable', () => {
      ...
      const client = new Storrik({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://api.storrik.io');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Storrik({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Storrik({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Storrik({ apiKey: 'My API Key' });
    expect(client2.maxRetries).toEqual(2);
  });

  describe('withOptions', () => {
    test('creates a new client with overridden options', async () => {
      const client = new Storrik({ baseURL: 'http://localhost:5000/', maxRetries: 3, apiKey: 'My API Key' });
    ...
    test('respects runtime property changes when creating new client', () => {
      const client = new Storrik({ baseURL: 'http://localhost:5000/', timeout: 1000, apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new Storrik({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new Storrik({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Storrik({ apiKey: 'My API Key', fetch: testFetch });
